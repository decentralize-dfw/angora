...
test('singleSided ships on, every unshipped task ships off', () => {
  ...
  // cinemaStill is OFF again, and this assertion is the guard that keeps it
  // off. The owner reported a black viewport twice: "hareket ettirince
  // gözüküyor, durunca siyah oluyor" - the idle accumulation takes the
  // screen when input stops. KAPANIŞ İŞ 3's "sample 0 = resolved frame"
  // seeding did NOT hold on a real GPU, and the fault does not reproduce
  // under the software rasteriser here, so no blind second fix was made.
  // A black screen costs more than a refined idle still. It ships again
  // only when the failure is reproduced and fixed on real hardware.
  // cinemaDof rides the same accumulation, so it goes with it.
  assert.equal(DEFAULT_FEATURES.cinemaStill, false);
  assert.equal(DEFAULT_FEATURES.cinemaDof, false);
