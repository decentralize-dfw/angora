...
function setup(baked=false){
 ...
 material.onBeforeCompile(shader,{});return {shader,fixture,light,controller,material};
}
...
test('Loading additional fixtures preserves previously compiled uniform capacities',()=>{
 const {shader,fixture,light,controller,material}=setup();
 controller.slots.push({light:new SpotLight(0xffffff,7)});
 assert.doesNotThrow(()=>fixture.update());
 assert.equal(shader.uniforms.fixturePosition.value.length,1);
 const next={uniforms:{},vertexShader:ShaderLib.standard.vertexShader,fragmentShader:ShaderLib.standard.fragmentShader};
 material.onBeforeCompile(next,{});
 assert.equal(next.uniforms.fixturePosition.value.length,2);
 light.intensity=3;fixture.update();
 assert.equal(shader.uniforms.fixtureRadiance.value[0].x,3);
 assert.equal(next.uniforms.fixtureRadiance.value[1].x,7);
});
