...
export function loadGradeTextures(rootURL) {
  ...
  // Every exterior family now ships the full PBR set: colour, relief and a
  // glTF metallicRoughness sheet (G roughness, B metalness). WebP, because
  // noise does not compress in PNG - the same 24 maps were 4.8 MB as PNG
  // and are 0.6 MB here. Generated by tools/textures/generate.py.
  const SHEETS = {
    clayTileMap: ['clay-tile-basecolor', 1], clayTileNormal: ['clay-tile-normal', 0],
    clayTileOrm: ['clay-tile-orm', 0],
    grassMap: ['grass-basecolor', 1], grassNormal: ['grass-normal', 0],
    grassOrm: ['grass-orm', 0],
    asphaltMap: ['asphalt-basecolor', 1], asphaltNormal: ['asphalt-normal', 0],
    asphaltOrm: ['asphalt-orm', 0],
    travertineMap: ['travertine-basecolor', 1], travertineNormal: ['travertine-normal', 0],
    travertineOrm: ['travertine-orm', 0],
    stuccoMap: ['stucco-basecolor', 1], stuccoMapSoft: ['stucco-basecolor-soft', 1],
    stuccoNormal: ['stucco-normal', 0], stuccoOrm: ['stucco-orm', 0],
    limestoneMap: ['limestone-basecolor', 1], limestoneNormal: ['limestone-normal', 0],
    limestoneOrm: ['limestone-orm', 0],
    timberMap: ['timber-basecolor', 1], timberNormal: ['timber-normal', 0],
    timberOrm: ['timber-orm', 0],
    metalNormal: ['metal-normal', 0], metalOrm: ['metal-orm', 0],
  };
  const names = Object.keys(SHEETS);
  return Promise.all(names.map(key => one(SHEETS[key][0] + '.webp', Boolean(SHEETS[key][1]))))
    .then(list => Object.fromEntries(names.map((key, i) => [key, list[i]])));
}
...
const BATCHED_TABLE = [
  ...
  // KAPANIŞ 4.3: ürün sahibi - "villa cephesinin rengi DOĞRU". İlk deneme
  // albedo + ortalama telafisiyle o rengi kaydırdı; artık STRUCCO albedo
  // BAĞLANMAZ. Düzlüğü kıran şey ORM (roughness varyasyonu) + kabartma.
  {name: 'STRUCCO', set: {normalMap: 'stuccoNormal', ormMap: 'stuccoOrm'},
   repeat: [0.64 / 1.5, 0.64 / 1.5], normalScale: 0.55},
  ...
  {name: 'metal', color: '#212326', dropMap: true,
   set: {normalMap: 'metalNormal', ormMap: 'metalOrm'}, repeat: [1.6, 1.6], normalScale: 0.5},
  {name: 'wood_dark.002', set: {map: 'timberMap', normalMap: 'timberNormal', ormMap: 'timberOrm'},
   repeat: [1.2, 1.2], normalScale: 0.7, keepAverage: true},
];
...
export function reviveBatchedGrade(models, sets, {anisotropy = 8, anyGrid = false, cellGrade = null} = {}) {
  ...
  for (const model of models.values()) {
    ...
    model.traverse(object => {
      ...
      // Scalars are not an alternative to sheets: the iron row keeps the
      // delivery's own colour AND takes a brushed response, so these run
      // for every entry that carries them, not only for the sheetless ones.
      ...
      // glTF packs roughness in G and metalness in B, which is what three's
      // roughnessMap and metalnessMap read - one sheet serves both. The
      // scalar factors MULTIPLY the sampled value, so they go to 1 or the
      // delivery's own numbers would halve the sheet. This is the flat
      // roughness fix: 164 of 177 source materials measured no variation
      // at all, and uniform specular reads as CG harder than flat colour.
      if (entry.set.ormMap) {
        const sheet = textureFor(entry.set.ormMap, entry.groundUV ? null : entry.repeat);
        if (sheet) {
          material.roughnessMap = sheet;
          material.metalnessMap = sheet;
          material.roughness = 1;
          material.metalness = 1;
          slots.push('roughnessMap', 'metalnessMap');
        }
      }
