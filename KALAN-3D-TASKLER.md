...
12. **Gerçek cihaz model bütçesi:** GLB’ler sadeleştirilmeden üretildi. R44 birleşik web teslimi üç parça ~27,1 MB ve bina 874 düğümdür; native tam ayrıntı ~105 MB olarak durur. Telefon için LOD/streaming kararı hâlâ gerçek FPS, bellek ve ısınma ölçümüyle verilmelidir.
13. **Bölge haritası yer imleri:** "Bölge" ölçeğindeki plan katmanı teslimin kendi verisinden üretilir; çevre yer imleri (`viewer/src/region-map.js` içindeki `LANDMARKS`) genel harita koordinatlarından yaklaşık girilmiştir ve arayüzde "yaklaşıktır" diye işaretlidir. Bilgi kaynağı olarak istenen `uzakolcek.html` hiçbir repoda bulunamadı; dosya eklendiğinde bu liste onun verisiyle değiştirilip mesafeler doğrulanmalıdır.
